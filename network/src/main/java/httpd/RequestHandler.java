	...
	public void run() {
		try {
			...
			while(true) {
				...
				//바디 시작 Simple HttpServer에서는 요청의 헤더만 읽음.
				...
				if("GET".equals(tokens[0])) 
				{
					...
					//SimpleHttpServer 에서는 무시(400 Bade Request 처리)
					//response400Error(outputStream, tokens[1], tokens[2])
				}
			// 예제 응답입니다.
			// 서버 시작과 테스트를 마친 후, 주석 처리 합니다.
//			respone400Error( outputStream, token[1], tocken[2] );
...
//			outputStream.write( "<h1>이 페이지가 잘 보이면 실습과제 SimpleHttpServer를 시작할 준비가 된 것입니다.</h1>".getBytes( "UTF-8" ) );
	...
	private void reponseStaticResource(OutputStream outputStream, String url, String protocol) throws IOException {
		...
		if(!file.exists()) {//response400Error(outputStream, url, protocol); 
			return;}
		//nio
